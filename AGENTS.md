...
- If the owner check is outstanding, hand off the ready preview and identify what remains unverified. After the check passes, leave the feature on its working branch for the owner-directed release flow below. A preview check is needed when the change warrants it, not for every dashboard edit.
...
- Commit coherent completed features or fixes on their working branch after the relevant verification. Keep completed work on that branch; task completion, passing checks and Preview approval do not authorize integration into `staging`.
- Merge selected working branches into `staging` only when the owner explicitly requests that integration as part of the release flow. Do not start staging integration as a routine task-completion step. Keep unfinished work separate.
...
- When the owner requests a Production release, fast-forward or merge `staging` into `master`, the source of Vercel Production deployments. Do not squash or rebase the long-lived `staging` branch into `master`; preserve their shared history for later releases.
