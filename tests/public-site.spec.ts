...
test("Home exposes session fees and the shared enquiry form", async ({ page }) => {
  await page.goto("/", { waitUntil: "networkidle" });

  const home = page.locator("main.home-page");
  const fees = home.getByRole("region", { name: "Sessions and fees" });
  const form = home.getByRole("form", { name: "Enquiry" });
  ...
  await expect(form.getByRole("link", { name: "find support now" })).toHaveAttribute(
    "href",
    "/crisis-support",
  );
...
test.describe("progressive enhancement", () => {
  ...
  test("Home exposes its native enquiry form without JavaScript", async ({ page }) => {
    await page.goto("/", { waitUntil: "networkidle" });
