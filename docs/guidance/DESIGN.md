...
Use this guide for public visual work, new components and shared-system maintenance. The private dashboard follows [Analytics](ANALYTICS.md).

## Visual Direction

- Public pages should belong recognisably to the same site, use shared implementation for recurring needs, and make room for creative composition and interesting new components. Use [website-design](../../.agents/skills/website-design/SKILL.md) for fresh creation or redesign, and consult the relevant [catalogues](../design-system/README.md).
...
- Keep genuinely unique work page-local. When the same role recurs, prefer a suitable shared implementation or a purposeful variant to near-duplicate components. Follow the shared-contract rules below; keep abstractions proportionate to demonstrated needs.
...
- `docs/design-system/FOUNDATIONS.md`, `docs/design-system/COMPONENTS.md`, and `docs/design-system/PATTERNS.md` own promoted reuse contracts. This document owns the compatibility permission below; the legacy register remains evidence only.
...
- `docs/design-system/FOUNDATIONS.md`: public semantic foundations such as approved colour or type roles, accessibility baselines, and genuinely shared primitives.
- `docs/design-system/COMPONENTS.md`: approved reusable React contracts, including their props, states, behaviour, accessibility, and supported boundaries.
- `docs/design-system/PATTERNS.md`: approved repeated semantic arrangements that coordinate foundations or components without imposing a page template.
...
6. When source is removed, delete its legacy-register entry. Git and `docs/memory/DECISIONS.md` retain durable history; do not create a retired-item catalogue.
...
- Render only items present in `docs/design-system/FOUNDATIONS.md`, `docs/design-system/COMPONENTS.md`, or `docs/design-system/PATTERNS.md`.
...
Follow [Verification](VERIFICATION.md#shared-design-changes) for documentation, shared implementation, consumer inspection and removal checks. Its development-only limits and the [analytics policy](ANALYTICS.md) apply.
...
- Update `docs/memory/DEBT.md` when unresolved migration or cleanup pressure should remain visible.
- Update `docs/memory/DECISIONS.md` for durable promotions, withdrawals, removals, or governance changes.
