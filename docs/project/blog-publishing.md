...
`src/content/blog/BlogArticleHero.tsx` is the supported reusable masthead template for every published article. It owns the breadcrumb, classification, sample label, title, abstract, author, dates, responsive composition, and its self-contained stylesheet; `BlogArticle.tsx` owns the publication note and return navigation around the body. The masthead composes the supported `.site-hero`, `.site-hero__eyebrow`, `.site-hero__statement`, and `.site-hero-background` roles while retaining its article-specific grid and metadata presentation. The registered presentation component owns only the article body. This keeps article orientation and publication details consistent while allowing structure and visual treatment below the hero to follow the subject of an individual article.
