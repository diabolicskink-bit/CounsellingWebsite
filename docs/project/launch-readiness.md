...
- `Current State`: Metadata exists and is prerendered, and `docs/checklists/seo-metadata-launch.md` is now the working checklist artifact. Home, Working with Joel, Inclusion, and Contact/Fees are configured as indexable on the apex custom domain, while the draft Inclusion child pages keep route-level `noindex, nofollow` metadata and stay out of sitemap output. A 2026-07-08 live SEO/discoverability review confirmed the four allowed pages pass indexability, canonical, sitemap, robots, redirect, not-found, and Lighthouse SEO/page-experience checks. The shared social preview image now exists at the configured metadata path, and generated homepage metadata now includes `WebSite` JSON-LD to give Google a preferred `Vive Counselling` site-name signal. All seven metadata-backed public routes now ship full component-rendered first-response HTML and matching-path hydration; the controlled 404 contract is locally verified, its post-deploy confirmation remains under `DEBT-24`, and temporary public-shell retirement remains under `DEBT-32` and `DEBT-33`. Remaining SEO/discoverability gaps are runtime metadata drift risk (`DEBT-27`), unconfirmed Search Console submission, absent Organization/Person/service structured data, and trust/practical-detail work tracked by `SITE-11`, `SITE-14`, `SITE-15`, `SITE-16`, and `SITE-17`.
