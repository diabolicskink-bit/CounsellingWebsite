...
- `Linked Work`: `DEBT-8`, `DEBT-27`, `DEBT-34`, `LAUNCH-8`, `SITE-11`, `SITE-14`, `SITE-15`, `SITE-16`, `SITE-17`, `SITE-23`
...
- `Current State`: Vercel Analytics remains enabled through the shared analytics gate only on allowed hostnames, Google Analytics sends explicit manual page views for public SPA route changes when configured, and Microsoft Clarity loads when `VITE_CLARITY_PROJECT_ID` is configured on an allowed hostname. Vercel receives property-free `enquiry_started` once per rendered form after the first non-honeypot input and `email_link_clicked` for site email-link clicks; neither intent event is sent to GA4. A confirmed `/api/enquiry` success emits GA4 `generate_lead` and Vercel `Enquiry submitted` events with no visitor-entered form data; failed submissions emit neither conversion event. The default host allowlist is the canonical apex domain plus `www`; preview/local analytics collection now requires an explicit `VITE_ANALYTICS_ALLOWED_HOSTS` override, which `npm run qa:analytics` supplies for local verification. No first-party cookie banner or local Clarity Consent API flow exists; the intended owner direction is to allow normal Clarity cookies where Microsoft/project settings allow them and rely on Clarity's regional no-consent behaviour where required. The enquiry form is explicitly Clarity-masked. Default QA stays analytics-quiet, while `npm run qa:analytics` verifies the enabled GA route-change path, anonymous contact-intent events and payloads, confirmed enquiry conversion path, failed-submission suppression, Vercel custom-event payload, and Clarity script load with third-party analytics URLs intercepted.
...
- `Links`: `.env.example`, `src/components/SiteAnalytics.tsx`, `src/components/EnquiryForm.tsx`, `src/utils/analytics.ts`, `tests/public-site.spec.ts`, `scripts/run-analytics-qa.mjs`
