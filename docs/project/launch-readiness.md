...
- `Current State`: Metadata exists and is prerendered, and `docs/checklists/seo-metadata-launch.md` is now the working checklist artifact. Home, Working with Joel, Inclusion, and Contact/Fees are configured as indexable on the apex custom domain, while the draft Inclusion child pages keep route-level `noindex, nofollow` metadata and stay out of sitemap output. A 2026-07-08 live SEO/discoverability review confirmed the four allowed pages pass indexability, canonical, sitemap, robots, redirect, not-found, and Lighthouse SEO/page-experience checks. The shared social preview image exists at the configured metadata path, and generated homepage metadata includes linked `WebSite`, `Organization`, credential-bearing `Person`, and `Service` JSON-LD. All seven metadata-backed public routes now ship full component-rendered first-response HTML and matching-path hydration; duplicate public H1 metadata and the temporary public-shell path are retired, while the controlled 404 uses a dedicated locally verified fallback whose post-deploy confirmation remains under `DEBT-24`. The rendering migration is accepted complete without a separate broad final-test campaign; relevant page assertions will be reviewed when those pages are already changing for other reasons under `DEBT-34`. Remaining SEO/discoverability gaps are runtime metadata drift risk (`DEBT-27`), unconfirmed Search Console submission, and trust/practical-detail work tracked by `SITE-11`, `SITE-14`, `SITE-15`, `SITE-16`, and `SITE-17`.
