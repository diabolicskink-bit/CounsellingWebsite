...
- `Next Action`: Add the Vite production server bundle and `entry-server.tsx` around the prepared static-router wrapper, then give the prerender script a durable component-render execution path before converting Home.
...
- `Notes`:
  ...
  - Phase 3 completed the shared application boundary: browser and static wrappers now supply different routers to the same route tree under the same Strict Mode boundary. The static wrapper is not yet executed by build tooling, so the temporary shell and `createRoot` behaviour remain unchanged.
