...
- Added `0010_add_phone_link_event.sql` and focused API, migration and browser contracts. The task recorded Preview at `0010` and Production at `0009` pending release. The [current release-dependency note](current-scope.md#runtime-configuration-and-deployment) retains the need to recheck this difference; this log is not a fresh database observation.
