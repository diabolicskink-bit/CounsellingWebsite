...
- Development-only routes include Documents, the Codex and Opus test beds, and the five `/design-language/*` pages. The design-language routes are quarantined as a browsable historical catalogue: every route inherits one visible archive notice and its content must not be treated as current production guidance or approved reusable API while `DEBT-37` remains open.
