...
- The five private views cover daily traffic, route totals, paid matched-keyword journeys, monthly enquiry outcomes, and excluded visitors. Daily reporting separates outbound email and social-profile clicks from enquiry outcomes, shows coarse location alongside device and automation context, and keeps the new events visible in each visit timeline. The Pages view attributes those outbound actions to their associated viewed route.
...
- `tests/analytics.spec.ts` is the separate private-analytics browser boundary. It covers host gating, private-route isolation, first-party collection, Fees attribution, visitor-ID rotation, GA4 and Clarity behaviour, outbound-action and coarse-location reporting, and a focused keyword-report journey.
