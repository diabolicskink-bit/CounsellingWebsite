...
- `/analytics` is an unlisted private route included in deployed builds and rendered outside the public-site navigation and footer. Vercel Routing Middleware protects it and the `/api/analytics` namespace with HTTP Basic Authentication supplied by server-only `ANALYTICS_USERNAME` and `ANALYTICS_PASSWORD` values; missing credentials fail closed, successful responses are non-cacheable and no-indexed, and direct navigation receives a dedicated no-index client shell. Local Vite development bypasses the platform middleware. The interface reads live retained data from the protected API, defaults to the current Australia/Perth day, provides date navigation, loading, failure, retry, and empty-day states, and uses summary panels with expandable visit rows. Expanding a row reveals its complete ordered page journey and attribution in place; a further action loads that anonymous browser's complete retained visit history with full detail for every visit.
...
- The browser recorder is separately gated by `VITE_VISIT_ANALYTICS_ENABLED` and its canonical-host allowlist. When enabled, it keeps a versioned anonymous visitor ID in `localStorage`, keeps the current visit in `sessionStorage`, starts another visit after 30 minutes of inactivity or a new external/tagged arrival, and retains the original landing path, full browser referrer, and recognized `gclid`, `ad`, `net`, `kw`, and `mt` attribution. Initial loads, full refreshes, and distinct React route pathnames each receive a duplicate-safe page-view ID under the active visit; hash/query-only changes and repeated React effects do not add page views. Private analytics paths are excluded from the first-party recorder, Google Analytics, and Microsoft Clarity so using the reporting interface does not record or expose its own activity through analytics providers. The source default remains off. The current Production deployment enables recording only on `vivecounselling.com.au` and `www.vivecounselling.com.au`; the protected `work/local-analytics` preview remains separately enabled, while other preview hosts remain off unless explicitly configured.
- A second database migration defines the read-only `visit_ledger` reporting view with retained-history new/returning status, visit sequence, traffic-source classification, elapsed time, and page totals. Five read-only Neon query templates provide a newest-first ledger, browser grouping, Australia/Perth daily overview, 30-day source/ad summary, and selected-visit page sequence. `npm run db:migrate` applies versioned files transactionally and tracks their filenames and checksums; both migrations are current in the shared Preview and Production database. The protected read-only `GET /api/analytics` accepts either a validated daily selection, defaulting to the current Australia/Perth date, or one validated anonymous visitor ID. It uses parameterized Neon queries to return newest-first visits with their complete ordered page journeys, full stored referrer and recognized ad attribution; database configuration and runtime failures remain generic to the client.
