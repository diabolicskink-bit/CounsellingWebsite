...
- Development-only routes include the rendered design-system pages, Documents, Policies, Codex test bed, and Opus test bed. The local-only Policies route is a restrained structural prototype with illustrative cancellation and confidentiality content; it is excluded from production builds, public metadata, prerendering, analytics, and the sitemap, and its footer link renders only in local development.
