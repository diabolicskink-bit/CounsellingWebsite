...
Public write endpoints share origin allowlists, strict Origin-header parsing, cross-site checks and blocked-request log sanitisation in [request-origin.ts](../../src/server/request-origin.ts). The request modules for enquiries, visits and visit events retain endpoint-specific body handling; page engagement uses the visits module. The visit-event policy preserves the separate local IPv6 difference tracked by `DEBT-47`.
