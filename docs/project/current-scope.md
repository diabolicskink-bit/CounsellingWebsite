...
- Direct Node tests under `tests/scripts/` cover route-metadata origin policy and the visit-ledger migration/query contracts.
...
- The inactive first-party visit endpoint has application-level request and payload controls but no platform rate limit. Production enablement remains gated by the visit-ledger plan's preview and owner-approval steps; platform rate limiting remains unresolved.
