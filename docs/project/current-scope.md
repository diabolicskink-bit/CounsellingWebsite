...
- Home uses a dark, direct opening with routes to its embedded enquiry form and the Inclusion hub, followed by one warm editorial About Vive section. A single uninterrupted narrative introduces Joel and online delivery, then develops general counselling for anxiety, depression, perfectionism and people-pleasing; trauma work including CPTSD, sexual trauma and single or repeated traumatic experiences; relationship work; and the practice's specialist and affirming relevance without internal subheadings. Detailed therapeutic methodology remains on Working with Joel. Its prose, specialist-practice introduction, and closing enquiry introduction use the shared reading role; the About opening alone uses the shared lead modifier. Joel's portrait panel contains the editorial link to Working with Joel, but the section intentionally contains no consultation promotion or conversion CTA. It leads into a dark Inclusive practice section that states Joel's relevant knowledge and lived experience, then gives Kink and BDSM, ethical non-monogamy and polyamory, and LGBTQIA+ counselling one direct-route surface with its own practice-specific scope copy; the three surfaces form an offset, overlapping composition without numbers or substitute category labels, while the general Inclusion hub link remains secondary. A separate compact Sessions and fees section then publishes the 50-minute individual and couples prices, free 15-minute initial consultation, and a `More than two?` action that moves directly to the Home enquiry form. The page closes with the same progressively disclosed appointment, consultation, and general-enquiry form used by Contact; its crisis-support boundary links to the dedicated Crisis Support route. Homepage arrow actions keep their container geometry fixed and use a consistent short movement for feedback. On non-hover or coarse-pointer input, the flat hero actions, portrait-panel link and specialist destination headings retain visible underlines and pressed feedback. On narrow screens, the complete About narrative precedes the portrait panel and its onward link, the specialist surfaces stack with a small alternating offset, the four fee items recompose into a two-by-two comparison, and the enquiry introduction stacks above the form.
...
- Contact/Fees uses the progressive form shared with Home, with three direct paths: make an appointment, request a consult, or make a general enquiry. Its first-message and crisis-support prose use the shared reading role, while fees, practical metadata, form copy, and submission feedback retain their specialised page roles. Both the form boundary and practical details point urgent visitors to the dedicated Crisis Support route. The shared form owns the browser submission workflow and reads its recipient and success content from `src/data/enquiry.ts`.
...
- The first-party visit ledger has a Neon/Postgres schema and a write-only `POST /api/visit` Vercel Function that accepts bounded anonymous visitor, visit, page-view, landing, full browser-supplied referrer, and optional ad-attribution fields. The endpoint derives the referrer hostname server-side, stores observations idempotently, exposes no read method, and returns generic public failures. Vercel manages one Neon resource connected to Development, Preview, and Production; its two migrations are current. The resource currently supplies the same database URL across Preview and Production.
- The browser recorder is separately gated by `VITE_VISIT_ANALYTICS_ENABLED` and its canonical-host allowlist. When enabled, it keeps a versioned anonymous visitor ID in `localStorage`, keeps the current visit in `sessionStorage`, starts another visit after 30 minutes of inactivity or a new external/tagged arrival, and retains the original landing path, full browser referrer, and recognized `gclid`, `ad`, `net`, `kw`, and `mt` attribution. Initial loads, full refreshes, and distinct React route pathnames each receive a duplicate-safe page-view ID under the active visit; hash/query-only changes and repeated React effects do not add page views. The source default remains off. The current Production deployment enables recording only on `vivecounselling.com.au` and `www.vivecounselling.com.au`; the protected `work/local-analytics` preview remains separately enabled, while other preview hosts remain off unless explicitly configured.
- A second database migration defines the read-only `visit_ledger` reporting view with retained-history new/returning status, visit sequence, traffic-source classification, elapsed time, and page totals. Five read-only Neon query templates provide a newest-first ledger, browser grouping, Australia/Perth daily overview, 30-day source/ad summary, and selected-visit page sequence. `npm run db:migrate` applies versioned files transactionally and tracks their filenames and checksums; both migrations are current in the shared Preview and Production database. No reporting data is exposed through the website or a public API.
- Anonymous browser IDs rotate on their 12-month calendar anniversary. A server-only `GET /api/visit-retention` function requires Vercel `CRON_SECRET` authorization and deletes visits older than Postgres `INTERVAL '12 months'`; their page views cascade-delete through the database relationship. `vercel.json` schedules it daily at 18:15 UTC (02:15 Australia/Perth). Preview and Production have separately generated sensitive secrets; the protected function has been exercised successfully in Preview, and the Production schedule is active with the visit-ledger release.
...
- `tests/public-site.spec.ts` is a single-Chromium, 46-case browser suite focused on durable visitor behaviour: public-route hydration and diagnostics, shared navigation and mobile focus/scroll restoration, Home fees and embedded-form behaviour, Crisis Support service destinations, complete urgent contact links, focus treatment, reduced-motion scrolling and semantic-colour cascade, SPA and not-found activation, JavaScript-disabled Contact and Home forms, lightweight first-response title/description/canonical and crawl-file checks, analytics privacy and events, redirects, approach-tab interaction, enquiry success/error behaviour, compact-viewport overflow, and route-level axe smoke checks. Detailed raw structure, structured/social metadata construction, duplicate output forms, asset bytes, and the controlled 404 artifact stay with the build generator and manual metadata monitor instead of being mirrored in browser assertions.
...
- The first-party visit endpoint has application-level request and payload controls but no platform rate limit. Its controlled preview flow is complete and production collection is active on the canonical hostnames; platform rate limiting remains unresolved.
