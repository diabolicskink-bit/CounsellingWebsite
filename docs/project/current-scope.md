...
- Development-only routes include the source-backed `/design-system` overview with separate `/design-system/foundations`, `/design-system/components`, and `/design-system/patterns` catalogue pages, plus Documents, the Codex and Opus test beds, and the standalone `/visit-report` interface mock. The former five `/design-language/*` rendered-catalogue routes remain retired and resolve through the ordinary Not Found route.
- `/visit-report` is an unlisted, development-only mock for the proposed private visit-reporting interface. It defaults to the current Australia/Perth day, uses explicitly fictional relative-date records, provides previous/next/date navigation and an empty-day state, and presents activity in a light open layout with pastel summary panels and expandable visit rows. The overview combines visit totals, returning-visitor and page-view figures, traffic-source proportions, and a chronological list with a compact preview of each visit's page sequence. Expanding a row reveals its complete page journey and attribution in place; a further action opens the anonymous browser's complete retained visit history with full detail for every visit. It deliberately has no database connection or authentication yet and renders outside the public-site navigation and footer.
...
- A second database migration defines the read-only `visit_ledger` reporting view with retained-history new/returning status, visit sequence, traffic-source classification, elapsed time, and page totals. Five read-only Neon query templates provide a newest-first ledger, browser grouping, Australia/Perth daily overview, 30-day source/ad summary, and selected-visit page sequence. `npm run db:migrate` applies versioned files transactionally and tracks their filenames and checksums; both migrations are current in the shared Preview and Production database. No real reporting data is exposed through the website or a public API; the development-only `/visit-report` route currently renders fictional local fixtures only.
