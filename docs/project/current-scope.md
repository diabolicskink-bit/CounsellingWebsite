...
- `/visit-report` is an unlisted private route included in deployed builds and rendered outside the public-site navigation and footer. Vercel Routing Middleware protects it and the `/api/visit-report` namespace with HTTP Basic Authentication supplied by server-only `VISIT_REPORT_USERNAME` and `VISIT_REPORT_PASSWORD` values; missing credentials fail closed, successful responses are non-cacheable and no-indexed, and direct navigation receives a dedicated no-index client shell. Local Vite development bypasses the platform middleware. The interface still uses explicitly fictional relative-date records and has no database reader connected yet. It defaults to the current Australia/Perth day, provides date navigation and an empty-day state, and uses pastel summary panels with expandable visit rows. Expanding a row reveals its complete page journey and attribution in place; a further action opens the anonymous browser's complete retained visit history with full detail for every visit.
...
- A second database migration defines the read-only `visit_ledger` reporting view with retained-history new/returning status, visit sequence, traffic-source classification, elapsed time, and page totals. Five read-only Neon query templates provide a newest-first ledger, browser grouping, Australia/Perth daily overview, 30-day source/ad summary, and selected-visit page sequence. `npm run db:migrate` applies versioned files transactionally and tracks their filenames and checksums; both migrations are current in the shared Preview and Production database. The protected read-only `GET /api/visit-report` scaffold accepts either a validated daily selection, defaulting to the current Australia/Perth date, or one validated anonymous visitor ID; it currently fails closed because its database reader is deliberately unimplemented. The `/visit-report` interface therefore continues to render fictional local fixtures only.
