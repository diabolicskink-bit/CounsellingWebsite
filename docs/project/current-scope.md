...
- The browser recorder is separately gated by `VITE_VISIT_ANALYTICS_ENABLED` and its canonical-host allowlist. When enabled, it keeps a versioned anonymous visitor ID in `localStorage`, keeps the current visit in `sessionStorage`, starts another visit after 30 minutes of inactivity or a new external/tagged arrival, and sends an initial observation containing the original landing path, full browser referrer, and recognized `gclid`, `ad`, `net`, `kw`, and `mt` attribution. It does not yet record subsequent React route changes. The flag defaults off, no Neon resource has been provisioned, and first-party visit collection is not active.
