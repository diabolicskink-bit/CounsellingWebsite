...
- Public routes include Home, Working with Joel, Inclusion, Kink and BDSM, ENM and polyamory, LGBTQIA+, Articles at `/blog`, one route per published article, Contact/Fees, and Not Found.
...
- Public desktop navigation includes Home, Working with Joel, Inclusion with its three child pages, Articles, and Fees, plus a separate Get in touch action. The mobile navigation exposes the same `/blog` route under the Articles label and keeps Fees and Contact separate. All shared Fees and Contact entry points open the Contact page at its top so visitors encounter its context before scrolling to the fee details. The compact shared footer keeps the wordmark, short navigation including Articles, email, hours, Instagram and LinkedIn profile links, and copyright separate from page-level CTA content.
...
- The Articles index at `/blog` uses a restrained publication composition with a factual introduction, chronological ruled article list, clear sample labels, and quiet links back to counselling information. Published articles use `/blog/:slug` and retain a consistent title, abstract, author, date, reading-time, publication-note, and return-navigation shell. Article bodies use the standard Markdown reading layout unless the post selects a registered page-scoped presentation. The ant and dinosaur sample articles demonstrate two different subject-led body designs without changing the shared shell.
- Articles are code-managed in `src/content/blog/posts.ts`. The typed registry validates unique URL-safe slugs and dates, sorts posts newest first, and supplies route and metadata generation. Registered sample articles remain publicly viewable but carry `noindex, nofollow` metadata and are excluded from the sitemap. `docs/project/blog-publishing.md` owns the publishing workflow, custom-presentation boundary, and future-CMS threshold; no CMS, database, login, draft state, or scheduling system is included.
...
- Core route metadata is stored in `src/data/routeMetadata.json`; article metadata is derived from the blog post registry, merged into the same runtime metadata map, and applied by `useDocumentMetadata`.
...
- All public content routes are indexable. Their generated HTML omits `noindex`, and `sitemap.xml` advertises the eight core canonical URLs plus every published article URL.
...
- `tests/public-site.spec.ts` covers public landmarks, raw and JavaScript-disabled output, hydration, shared mobile-navigation Escape/focus/scroll-lock behaviour including responsive breakpoint release, complete mobile-footer scroll reach, focused Home semantics, CTA ownership, dark-hero contrast, About Vive content, mobile portrait ordering and overflow, Working with Joel hero alignment, heading roles, tab interaction and axe checks, Articles index/article rendering and wayfinding, subject-specific sample presentations, unknown article slugs, article metadata, sample noindex behaviour, sitemap inclusion, header/mobile/footer destinations, timezone behaviour, three-path conditional enquiry fields and payloads, success/error states, form semantics, flat and nested artifacts, SPA navigation, fallback activation, generated metadata, sitemap, robots, and the 404 artifact.
