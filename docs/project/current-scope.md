...
- The `/design-system` workspace is a rendered view of the current-only catalogues. Foundations renders the promoted `--cedar`, warm-section, and portrait-material tokens; Patterns renders `.site-section-warm` and the shared `.site-hero-background` used by every public-route hero and every development hero or cover. Only `docs/design-system/foundations.md`, `components.md`, and `patterns.md` authorize reuse; inherited implementation remains absent from the workspace and is recorded separately under `docs/design-system-legacy/` when useful.
