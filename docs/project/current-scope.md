...
Public write endpoints share origin allowlists, strict Origin-header parsing, cross-site checks and blocked-request log sanitisation in [request-origin.ts](../../src/server/request-origin.ts). The request modules for enquiries, visits and visit events retain endpoint-specific body handling; page engagement uses the visits module. Visit-event collection supports IPv4 loopback for local development and does not require local IPv6 support.
