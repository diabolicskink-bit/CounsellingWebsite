...
**Recorded release dependency:** the repository has eleven ordered migrations through `0011_add_consult_cta_event.sql`. Preview is current through `0011`; Production remains current through `0010_add_phone_link_event.sql` until the release migration is applied. This is a retained operational observation from 2026-09-10, not a fresh database check. Confirm Production before releasing consult-CTA event collection, and update or remove this note when the difference is resolved. The dated [consult CTA](task-log.md#2026-09-10---consult-cta-first-party-event-added) and [phone analytics](task-log.md#2026-09-09---contact-phone-analytics-added) entries retain the history.
