...
- A second database migration defines the read-only `visit_ledger` reporting view with retained-history new/returning status, visit sequence, traffic-source classification, elapsed time, and page totals. Five read-only Neon query templates provide a newest-first ledger, browser grouping, Australia/Perth daily overview, 30-day source/ad summary, and selected-visit page sequence. `npm run db:migrate` applies versioned files transactionally and tracks their filenames and checksums; both migrations are current in the shared Preview and Production database. The protected read-only `GET /api/analytics` scaffold accepts either a validated daily selection, defaulting to the current Australia/Perth date, or one validated anonymous visitor ID; it currently fails closed because its database reader is deliberately unimplemented. The `/analytics` interface therefore continues to render fictional local fixtures only.
