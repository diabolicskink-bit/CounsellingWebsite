# SEO Google Search Result Previews

This is the Google-style search-result preview companion for `LAUNCH-3`.

These examples are based on the current route metadata and canonical launch origin. They are not guaranteed search results. Google may rewrite titles or snippets, vary them by query, add sitelinks, or omit pages that are not indexed.

Use [seo-metadata-launch.md](seo-metadata-launch.md) for checklist status and [seo-metadata-inventory.md](seo-metadata-inventory.md) for field-by-field metadata.

## Indexable Launch Pages

### `/` - Home

```text
Online Therapy Across Australia | Vive Counselling
https://vivecounselling.com.au/
Online counselling and psychotherapy across Australia. Support for anxiety, depression, relationships and trauma. Kink, ENM and LGBTQIA+ aware.
```

### `/working-with-joel` - Working with Joel

```text
Joel Griffiths, Psychodynamic Counsellor | Vive Counselling
https://vivecounselling.com.au/working-with-joel
Psychodynamic counselling with Joel Griffiths, an ACA-registered counsellor working online across Australia, based in Perth. Attachment-informed and integrative.
```

### `/inclusion` - Inclusion Hub

```text
Kink, ENM and LGBTQIA+ Inclusive Counselling | Vive Counselling
https://vivecounselling.com.au/inclusion
Inclusive counselling for adults across Australia, with particular understanding of kink, BDSM, ENM, polyamory and LGBTQIA+ lives. Perth-based and non-shaming.
```

### `/contact` - Contact/Fees

```text
Counselling Fees & Contact | Vive Counselling
https://vivecounselling.com.au/contact
Request a free 15-minute initial consultation with Joel Griffiths at Vive Counselling. Counselling sessions are $120; view fees and contact details.
```

## Indexable Inclusion Child Pages

These routes are linked in production, included in the sitemap, and use the same indexable metadata policy as the other public content routes.

### `/inclusion/kink-bdsm` - Kink and BDSM

```text
Kink & BDSM-Aware Counselling | Vive Counselling
https://vivecounselling.com.au/inclusion/kink-bdsm
Kink-aware counselling and therapy with Joel Griffiths, an ACA-registered counsellor based in Perth and working with adults across Australia.
```

### `/inclusion/enm-polyamory` - ENM and polyamory

```text
Counselling for Polyamory, Open Relationships and ENM | Vive Counselling
https://vivecounselling.com.au/inclusion/enm-polyamory
Counselling for polyamory, open relationships and ENM across Australia, without assuming monogamy is the answer or treating relationship structure as the problem.
```

### `/inclusion/lgbtqia` - LGBTQIA+

```text
LGBTQIA+ Affirming Counselling | Vive Counselling
https://vivecounselling.com.au/inclusion/lgbtqia
LGBTQIA+ affirming counselling with Joel Griffiths for adults across Australia. Perth-based online counselling for sexuality, gender, relationships and other concerns.
```
