...
The new development-only `/design-system` route is a source-backed workspace. Foundations contains six supported specimens: `--cedar`, the `--section-warm` and `--section-rule` editorial-section foundations, and the `--portrait-panel`, `--portrait-frame`, and `--portrait-footer-tint` material set. Patterns contains the supported `.site-section-warm` contract used across all public routes. All other unreviewed inventory remains absent. The workspace explains the promotion gate, links into the written records through Documents, and provides a strict specimen frame for promoted items. It is a view over authority rather than a catalogue register or a continuation of the retired snapshot.
