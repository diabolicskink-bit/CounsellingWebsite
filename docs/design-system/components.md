...
- `Implementation`: `ContactInvitation` in `src/components/ContactInvitation.tsx` with `.contact-invitation*` styles in `src/styles.css`.
- `Verified consumers`: Home, as the final section before the shared footer.
