...
- `Boundary`: Owns its section semantics; fixed consult, email, fee, and supporting copy; Contact anchors and action labels; dark editorial surface; responsive layout; focus, pointer, and reduced-motion behaviour. Consumers own placement only. It accepts no props, appears at most once as the final section of a public page, and is not intended for Contact or development routes.
