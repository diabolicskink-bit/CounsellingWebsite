...
- `--section-warm` is `Shared-supported` as the flat warm background for verified primary light editorial sections across Home, Working with Joel, Inclusion, Contact, Kink and BDSM, ENM and polyamory, and LGBTQIA+. The former Working with Joel introduction glow has been removed; the Inclusion chapters retain their page-local sage left-hand overlay.
- `--section-rule` and `.site-section-warm` are `Shared-supported` as the shared boundary and complete warm editorial-section contract. The pattern owns its material, `60px` default vertical rhythm, and lower rule while leaving content composition and verified responsive padding exceptions page-local.
...
- The `/design-system` route, `DesignSystemSpecimen`, and their page-scoped styling are `Historical/dev-only` workspace support. The route renders six source-backed Foundations specimens and one Patterns specimen without becoming lifecycle authority itself.
...
- Production colour, typography, spacing, radius, shadow, surface, and layout tokens are defined in `src/styles.css`; the supported warm-section pattern aligns the verified light editorial sections' material, default spacing, and boundary across public routes, while the supported portrait-material tokens replace duplicate colour declarations on Home and Working with Joel.
...
- The development Design System route uses a page-scoped governance-ledger composition, links to authoritative Markdown records through Documents, and renders six explicitly `Shared-supported` Foundations tokens plus the first supported Patterns specimen.
...
- Supported specimens beyond the six promoted Foundations tokens and `.site-section-warm`; no other item has completed promotion under the current lifecycle.
