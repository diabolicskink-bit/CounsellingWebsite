...
- The active shared hero system uses `hero-*` classes, including `.hero-section`, `.hero-bg--default`, `.hero-top`, `.hero-display`, `.hero-intro`, `.hero-copy-panel`, `.hero-deck`, `.hero-support-tagline`, `.hero-media-note`, `.hero-principles-strip`, and `.hero-detail-stack`; `.hero-section` owns the shared 40px top and bottom spacing, and `.hero-display` owns the measured shared page-opening H1 typography and default title measure.
