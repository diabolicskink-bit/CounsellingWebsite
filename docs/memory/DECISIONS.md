...
Consequential decisions and milestones that help later work. Git retains detailed implementation history and earlier versions of these entries. This file holds the current month's milestones; completed months live in the [history archive](archive/README.md#task-history).
...
- Use [SYSTEM.md](../reference/SYSTEM.md) for current system understanding and source owners, [AGENTS.md](../../AGENTS.md) for working rules, and the [project map](../README.md) for specialist guidance. Dated descriptions of old workflows or design contracts do not override their current owners.
- Use [project debt](DEBT.md) and the [site backlog](BACKLOG.md) for outstanding work. Historical verification and environment statements describe observations at that date, not present deployment status.
- Search this file and `archive/DECISIONS-*.md` together when a topic spans months. The archive index gives each month's main subjects.
...
- When adding the first entry for a new month, move completed months to `archive/DECISIONS-YYYY-MM.md` and update the archive index. Preserve links to moved entries or update their incoming references.
...
## 2026-09-18 - Instruction Structure Rebuilt Around Task Owners

- Rebuilt [AGENTS.md](../../AGENTS.md) as the core working agreement and direct task router. [Guidance](../README.md#task-guidance) now owns domain decisions and procedures, [reference](../reference/SYSTEM.md) owns system/practice facts, and this directory owns project memory. Existing release permissions, private analytics boundaries, public-writing preferences and the agreed design/reuse direction remain in force.
- Adopted uppercase document basenames with lowercase folders and `.md` extensions. Updated cross-references and the development document reader, including mechanical skill links; skill workflows and the CSS review were retained. The shared identity compatibility allowance remains explicit while source reconciliation continues.

## 2026-09-18 - Visual Guidance Balanced Identity, Reuse And Invention

- The owner clarified that public pages should share a recognisable identity and maintainable recurring components while encouraging interesting new components and creative composition. [Visual guidance](../guidance/DESIGN.md#visual-direction) now makes suitable reuse and content-led invention normal choices, replacing mandatory departure from existing page patterns.
- [Governance](../guidance/DESIGN.md#public-identity-compatibility-baseline) grants a bounded compatibility allowance for existing identity tokens, the application shell, containment and standard actions so ordinary page work can reuse them directly. The allowance does not promote the remaining legacy system. Directly needed shared variants or extractions are included in ordinary page work, with contract updates and affected-consumer verification; identity changes and broad migrations remain separately scoped.

## 2026-09-17 - AI Guidance Consolidated By Responsibility

- Kept task interpretation, permissions, verification scope, private analytics policy, and creative direction in [AGENTS.md](../../AGENTS.md). Detailed test standards and execution guidance now live in the [test guide](../../tests/README.md); shared-system lifecycle rules live in [governance](../guidance/DESIGN.md).
...
- Added the private Referrers report using the Pages interaction model. The owner selected all-visit coverage, combined `www.` variants, visit-count ranking and enquiry-visit counts without enquiry percentages. [Current scope](../reference/SYSTEM.md#reporting-semantics) owns the reporting definitions.
...
- Rebuilt [VERIFICATION.md](../guidance/VERIFICATION.md) around readiness, grouped captures and cause-based troubleshooting; [AGENTS.md](../../AGENTS.md) retained ownership of verification scope.
...
- Rebuilt [writing direction](../guidance/WRITING.md) around natural writing, the commercial role of practice/service copy, proportionate factual checks and editorial freedom. Removed standing page-content hierarchies and placement instructions.
- Added [ARTICLES.md](../guidance/ARTICLES.md#writing-an-article) for authoring, evidence and APA references, separating it from [publication procedures](../guidance/ARTICLES.md#implementing-and-publishing). Recorded the owner's article preference as a blend of postgraduate research writing and a compelling blog article.
...
- Replaced `practice-direction.md` with [PRACTICE.md](../reference/PRACTICE.md), preserving practice facts, expertise, lived experience, commitments and approach while removing messaging priorities and page-placement rules.
...
- Rebuilt [SYSTEM.md](../reference/SYSTEM.md) around capabilities, connected flows, source ownership, runtime/privacy boundaries and verification instead of granular page-presentation descriptions.
...
- Separated form sends, phone enquiries, failed outcomes and form send rate; a number click did not prove a call was placed or answered. Current definitions belong to the [analytics system guide](../reference/SYSTEM.md#analytics-and-data-meaning).
...
- Updated [article publishing](../guidance/ARTICLES.md#implementing-and-publishing), the system guide and ArticleHero's [component contract](../design-system/COMPONENTS.md) to match the simplified model.
...
- Expanded practice, practitioner, service and offer descriptions in structured data. Current emitted metadata and its owners are mapped in [current scope](../reference/SYSTEM.md#rendering-routing-and-discoverability).
...
- Set the visible/metadata title to `Kink-Aware Therapy and the Psychology of BDSM`, replaced the abstract/search description and preserved the canonical route and recorded substantive revision date. [Article publishing](../guidance/ARTICLES.md#implementing-and-publishing) owns the content workflow.
...
- Added visible selected-reference feedback after in-page jumps. The citation and reference model is documented in [article publishing](../guidance/ARTICLES.md#implementing-and-publishing).
...
- Extended protected report shapes, SQL and focused checks while preserving owner-only access. Current data meanings belong to the [analytics system guide](../reference/SYSTEM.md#analytics-and-data-meaning).
...
- Aligned collection/article structured data, navigation, analytics, prerendering and sitemap handling. Breadcrumb hierarchy was subsequently corrected on 9 September; [article publishing](../guidance/ARTICLES.md#implementing-and-publishing) describes the current contract.
...
- Restricted saving to the selected article's body/reference template through a localhost-only Vite endpoint. Metadata stayed source-only and the editor was excluded from Production; [article publishing](../guidance/ARTICLES.md#implementing-and-publishing) owns editing boundaries.
...
- Current content structure and reference rules belong to [article publishing](../guidance/ARTICLES.md#implementing-and-publishing) and [article writing](../guidance/ARTICLES.md#writing-an-article).
...
- Current naming and reuse authority belong to [design governance](../guidance/DESIGN.md) and its active catalogues.
...
- Validated required metadata, ordered real dates, matching bodies and sitemap modification dates, with focused script/browser coverage. The then-supported custom-presentation field was removed on 9 September; [article publishing](../guidance/ARTICLES.md#implementing-and-publishing) owns today's schema.
