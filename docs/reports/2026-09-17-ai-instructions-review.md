...
| `docs/project/README.md` | Map documents and explain ownership | Useful map; repeats several policies already stated at the root |
...
**Evidence.** The [system guide's route table](../project/current-scope.md#private-reporting) says `/analytics/enquiries` includes successful sends and phone- and email-click signals. Its [reporting semantics](../project/current-scope.md#reporting-semantics), line 195, then describes successful form sends and phone clicks as enquiry signals and says email/social clicks remain separate outbound actions.
...
**Evidence.** [AGENTS.md](../../AGENTS.md), lines 41–44, calls for the smallest useful checks and says development-only presentation changes normally need source inspection and the smallest relevant static check. The [design-system governance verification section](../design-system/governance.md#verification), lines 119–120, instead requires a full build for CSS, component, or rendered-workspace changes unless the task explicitly excludes it, and direct inspection for visual or interaction changes.
