...
name: impeccable
description: >
  Explicit invocation only. Use this skill only when the user invokes
  `$impeccable` or `/impeccable`, or asks to use Impeccable by name. Do not
  auto-trigger it for ordinary frontend design, implementation, review,
  polish, audit, accessibility, performance, responsive, copy, or
  design-system work. Once explicitly invoked, use the matching Impeccable
  workflow for frontend interfaces. Not for backend-only or non-UI tasks.
---
